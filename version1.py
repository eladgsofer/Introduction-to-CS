...
class GameOfLife(game_of_life_interface.GameOfLife):
    ...
    def __init__(self, board_size, starting_position=1, rules='B3/S23'):
        ...
        self.rules = rules
        self.born, self.survive = [(int(r[1:]) * self.ALIVE,) if len(r) == 2
                                   else (int(r[1:2]) * self.ALIVE, int(r[2:]) * self.ALIVE)
                                   for r in rules.split('/')]

        self.board = self.board = np.random.choice([0], size=self.nxn)
        self.__init_board()

        plt.interactive(True)
        self.display = plt.matshow(self.board, fignum=self.FIGNUM)
    ...
    def update(self):
        ...
        for r_index, r   in enumerate(self.board):
            ...
            for c_index, cell  in enumerate(r):
                ...
                if r_index in self.edges or c_index in self.edges:
                    # Row borders
                    r_f = 0 if r_index-1<0 else r_index-1
                    r_l = r_index if r_index+1==self.board_size else r_index+1
                    # Column borders
                    c_f = 0 if c_index-1<0 else c_index-1
                    c_l = c_index if c_index+1==self.board_size else c_index+1
        ...
        # plt.matshow(self.return_board(), fignum=0)
    ...
    def display_board(self):
        ...
        '''
        self.display = plt.matshow(self.return_board(), fignum=self.FIGNUM)
        plt.show()
